...
BOOL SphereHitable::Hit(const Ray &r, float t_min, float t_max, HitRecord &out_rec) const
{
	...
	if (m_motion)
	{
		center = m_motion->Move(m_center, r.m_emittedTimeInMs / 1000.0f);
	}
	else
	{
		center = m_center;
	...
	if (discriminant > 0)
	{
		...
		if (t < t_max && t > t_min)
		{
			...
			out_rec.m_normal = (out_rec.m_position - center) / m_radius; // same as normalize, cos the length is know as m_radius
...
void SphereHitable::OnUpdate(SimpleCamera *camera, float elapsedSeconds)
{
	if (m_motion)
	{
		m_center = m_motion->Move(m_center, elapsedSeconds);
	}
}
